    ...
    public sealed class SceneTopDownMapUI : MonoBehaviour
    {
        ...
        private void ScanSceneEntities()
        {
            ...
            foreach (MonoBehaviour mb in allScripts)
            {
                ...
                // Terminali e datapad restano fuori mappa: sono indizi di gameplay, non marker tattici.
                else if (mb is TerminalePorta || mb is DatapadCodiciPorte)
                {
                    continue; // salta, non tracciare i pannelli sulla mappa
                }
                // L'uscita non viene marcata in anticipo: deve diventare chiara solo a obiettivo risolto.
                else if (mb is IInteractable)
                {
                    string rawLower = t.gameObject.name.ToLower();
                    if (!rawLower.Contains("light") && !rawLower.Contains("cam") && !rawLower.Contains("audio") && !rawLower.Contains("sound") && !rawLower.Contains("volume") && !rawLower.Contains("vfx") && !rawLower.Contains("particle") && !rawLower.Contains("particelle") && !rawLower.Contains("fx") && !rawLower.Contains("spark"))
                    {
                        isInteractable = true;
                ...
                {
                    string rawLower = t.gameObject.name.ToLower();
                    if (!rawLower.Contains("light") && !rawLower.Contains("cam") && !rawLower.Contains("audio") && !rawLower.Contains("sound") && !rawLower.Contains("vfx") && !rawLower.Contains("particle") && !rawLower.Contains("particelle") && !rawLower.Contains("fx") && !rawLower.Contains("spark"))
                    {
                        isInteractable = true;
                        nomeTattico = CleanObjectName(t.gameObject.name);
                    }
                ...
                if (isInteractable)
                {
                    ...
                    if (!isDuplicate)
                    {
                        ...
                        if (nomeTattico.Length > 18) nomeTattico = nomeTattico.Substring(0, 18).Trim();
        ...
        public static string CleanDoorName(string raw)
        {
            if (string.IsNullOrWhiteSpace(raw)) return "PRT";
            ...
            n = AbbreviaTestoTattico(n);
            if (n.Contains("PORTA") || n.Contains("PRT") || n.Contains("DOOR") || n.Contains("GATE") || n.Contains("SET.")) return n;
            return "PRT " + n;
        }
        public static string CleanObjectName(string raw)
        {
            if (string.IsNullOrWhiteSpace(raw)) return "INTERAGIBILE";
            // Rimuove prefissi e suffissi tipici dei modelli 3D e unity
            string n = raw.ToUpper().Replace("(CLONE)", "").Replace("_", " ").Replace("-", " ").Trim();
            n = System.Text.RegularExpressions.Regex.Replace(n, @"(?i)\b(DEFAULTMATERIAL|MATERIAL|MESH|PREFAB|LOD\d*|SM|ENV|PROP|POLY|LOWPOLY)\b|\(\d+\)", "").Trim();
            ...
            n = n.Replace("CUBE", "CASSA")
                 ...
                 .Replace("LIGHT", "ILLUMINAZIONE");
            // Pulisce spazi doppi e numeri isolati rimasti (es. "PORTA 04" -> "PORTA")
            n = System.Text.RegularExpressions.Regex.Replace(n, @"\b\d{1,3}\b", "").Trim();
            n = System.Text.RegularExpressions.Regex.Replace(n, @"\s+", " ").Trim();
            n = AbbreviaTestoTattico(n); // comprime per UI
            if (string.IsNullOrWhiteSpace(n)) return "INT.";
            return n;
        }
        public static string AbbreviaTestoTattico(string n)
        {
            return n.Replace(" DI ", " ")
                    .Replace(" DEL ", " ")
                    .Replace(" DELLA ", " ")
                    ...
                    .Replace(" IL ", " ")
                    .Replace(" LA ", " ")
                    .Replace("PANNELLO", "PAN.")
                    .Replace("CONTROLLO", "CTRL")
                    .Replace("STANZA", "S.")
                    .Replace("SETTORE", "SET.")
                    .Replace("PRINCIPALE", "PRINC.")
                    .Replace("SICUREZZA", "SICUR.")
                    .Replace("GENERATORE", "GEN.")
                    .Replace("PORTA", "PRT")
                    .Replace("DATA KEY", "CHIAVE")
                    .Replace("DATAPAD", "PAD")
                    .Replace(" PORTATILE", "")
                    .Replace(" MATERIALI", "")
                    .Trim();
