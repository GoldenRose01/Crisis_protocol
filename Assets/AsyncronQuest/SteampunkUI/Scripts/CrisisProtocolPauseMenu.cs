...
namespace AsyncronQuest.SteampunkUI
{
    public sealed class CrisisProtocolPauseMenu : MonoBehaviour
    {
        ...
        private void SetPaused(bool value, bool force = false)
        {
            ...
            if (menuGroup)
            {
                menuGroup.gameObject.SetActive(isPaused);
        ...
        // Chiude la mappa senza lasciare overlay appesi quando si torna al menu.
        private void SpegniMappaPausaSubito()
        {
            isPaused = false;
            ModalUIState.Close(ModalOwner);
        ...
        private void BuildInterface()
        {
          ...
          // 1. Create the map group under the canvas root
            ...
            resumeButtonRect = AddNeonButton("Btn_Resume", "[ RIPRENDI (ESC) ]", actionsBar.transform, new Vector2(-160f, 22f), new Vector2(290f, 46f), new Color(0.0f, 1.0f, 0.5f), new Color(0.0f, 0.24f, 0.11f, 1f), Resume).GetComponent<RectTransform>();
            exitButtonRect = AddNeonButton("Btn_Exit", "[ MENU PRINCIPALE ]", actionsBar.transform, new Vector2(160f, 22f), new Vector2(290f, 46f), new Color(1.0f, 0.18f, 0.18f), new Color(0.32f, 0.0f, 0.0f, 1f), BackToMainMenu).GetComponent<RectTransform>();
        ...
        private Button AddNeonButton(string objectName, string label, Transform parent, Vector2 position, Vector2 size, Color neonColor, Color sfondoForte, UnityEngine.Events.UnityAction action)
        {
            ...
            Shadow ombraTesto = txtObj.AddComponent<Shadow>();
            ombraTesto.effectColor = Color.black;
            ombraTesto.effectDistance = new Vector2(2f, -2f);
            return button;
        }
