...
namespace AsyncronQuest.SteampunkUI
{
    ...
    public sealed class AsyncronQuestSteampunkUI : MonoBehaviour
    {
        ...
        private bool bloccoInputAperturaMenu = false;
        private float sbloccoInputMenuAt = 0f;
        private void Awake()
        {
            Time.timeScale = 1f;
        ...
        private void Update()
        {
            ...
            AggiornaBloccoInputAperturaMenu();
        }
        ...
        // Spegne il menu prima del cambio scena, cosi' non flasha sopra terminali o briefing.
        private void NascondiMenuDuranteAvvio()
        {
            if (mainCanvas != null) mainCanvas.enabled = false;
            if (mainCanvasGroup != null) mainCanvasGroup.blocksRaycasts = false;
        ...
        // Anti-doppio click: evita che il click ereditato prema subito un bottone del menu.
        private void BloccaInputMenuAppenaAperto()
        {
            bloccoInputAperturaMenu = true;
            sbloccoInputMenuAt = Time.unscaledTime + 0.35f;
            if (mainCanvasGroup == null) return;
            mainCanvasGroup.alpha = 1f;
            mainCanvasGroup.interactable = false;
            mainCanvasGroup.blocksRaycasts = false;
        }
        // Riattiva input solo dopo il mini cooldown e il rilascio del mouse.
        private void AggiornaBloccoInputAperturaMenu()
        {
            if (!bloccoInputAperturaMenu) return;
            if (Time.unscaledTime < sbloccoInputMenuAt) return;
            if (Input.GetMouseButton(0)) return;
            bloccoInputAperturaMenu = false;
            if (mainCanvasGroup == null) return;
