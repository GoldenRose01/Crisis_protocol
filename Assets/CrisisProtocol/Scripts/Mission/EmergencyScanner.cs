    ...
    private void AnalizzaBersaglio(Collider target)
    {
        ...
        if (hotspot != null)
        {
            ...
            if (possiedeChiave)
            {
                ...
                Debug.Log($"<color=yellow>[SCANNER]</color> Focolaio analizzato: <b>{cleanName}</b> [{stato}] - Chiave [{reqKey}] abilitata.");
                if (CyberHUD.Instance != null)
                {
                    CyberHUD.Instance.MostraNotificaAcquisizione($"⚠️ {cleanName}", $"{stato}");
                }
            }
            else
            {
        ...
        if (isHighlightActive && telecameraPrincipale != null)
        {
            ...
            for (int i = 0; i < activeTargetHighlights.Count; i++)
            {
                ...
                if (screenPos.z > 0f)
                {
                    ...
                    string testoMarker = $"{target.label} [{Mathf.RoundToInt(distance)}m]";
                    float larghezzaBox = Mathf.Clamp(labelStyle.CalcSize(new GUIContent(testoMarker)).x + 44f, 320f, Mathf.Min(620f, Screen.width - 32f));
                    float altezzaBox = Mathf.Clamp(labelStyle.CalcHeight(new GUIContent(testoMarker), larghezzaBox - 24f) + 16f, 42f, 74f);
                    float boxX = Mathf.Clamp(screenPos.x - (larghezzaBox * 0.5f), 16f, Screen.width - larghezzaBox - 16f);
                    float boxY = Mathf.Clamp(guiY - altezzaBox - 10f, 16f, Screen.height - altezzaBox - 16f);
                    Rect markerRect = new Rect(boxX, boxY, larghezzaBox, altezzaBox);
                    ...
                    GUI.Label(markerRect, testoMarker, labelStyle);
                }
            }
        }
    }
