...
public class TerminalePortaUI : MonoBehaviour
{
    ...
    public void ConfermaCodice()
    {
        ...
        // Verifica quarantena
        if (terminaleAttivo.richiedeEmergenzaRisolta && MissionManager.Instance != null && !MissionManager.Instance.EstrazioneSbloccata)
        {
            StartCoroutine(SequenzaErrore("IMPOSSIBILE APRIRE // EMERGENZA IN CORSO"));
            return;
        }
    ...
    {
        // Scelta del prossimo nodo: semplice ma leggibile, tipo bivio su carta.
        switch (nodoCorrente)
        {
            case 1:
    ...
    public void TentaBypassNodo()
    {
        ...
        if (terminaleAttivo != null && terminaleAttivo.richiedeEmergenzaRisolta && MissionManager.Instance != null && !MissionManager.Instance.EstrazioneSbloccata)
        {
            StartCoroutine(FlashNodo(false));
            if (testoStatoMessaggio != null) {
                testoStatoMessaggio.text = "✗ IMPOSSIBILE APRIRE // EMERGENZA IN CORSO";
                testoStatoMessaggio.color = Color.red;
            }
            return;
        }
